...
public:
    ...
    void processQuery(const Query& query) {
        if (query.type == "check") {
			...
			list<string> list_to_print= elems[query.ind];
			for (auto it = list_to_print.rbegin(); it != list_to_print.rend(); ++it) 
					std::cout << *it << " ";
            std::cout << "\n";
        } else {
            list<string>::iterator it = std::find(elems[hash_func(query.s)].begin(), elems[hash_func(query.s)].end(), query.s);
            if (query.type == "find")
